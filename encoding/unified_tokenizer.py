...
def multi_token_from_token(token):
    return AbstractMultiToken(
        spellings=(token.spelling,), kind=token.kind, metadata=token.metadata
    )


# TODO(maniatis): Add a test for this one, and migrate other copies to use
# the same implementation.
def fill_range_with_whitespace(start, end):
    ...
    if (current_line, current_column) >= (end_line, end_column):
        raise ValueError(
            "`start` must precede `end`, but we received start %s "
            "and end %s." % (start, end)
        )
...
_CAMEL_AFTER_SNAKE_EXPRESSION = "|".join(_CAMEL_AFTER_SNAKE_EXPRESSIONS).format(
    L=_LOWERCASE, T=_TITLECASE, U=_UPPERCASE
)
_CAMEL_RE = regex.compile(
    _CAMEL_AFTER_SNAKE_EXPRESSION, regex.U
...
def code_to_tokens(code):
    ...
    Raises:
      ...
      IndentationError: When the source code is incorrectly indented. This is
        generated by `tokenize.generate_tokens`.
    """
    token_tuples = list(tokenize.generate_tokens(six.StringIO(code.rstrip()).readline))
    logging.vlog(5, "Code `%s` was tokenized to token tuples `%s`.", code, token_tuples)
    ...
    if len(token_tuples) > 2:
        ...
        if current and token_tuples[current][0] == tokenize.NEWLINE:
            ...
            logging.vlog(
                5,
                "Tokenization for `%s` was sanitized to remove "
...
def code_to_tokens_simple_lossless(code):
    ...
    for i, inputchar in enumerate(code):
        ...
        if (state, next_state) in normal_transitions:
            ...
            memory = [memory[-1]]
        elif (state, next_state) == (StateType.SPECIAL_STATE, StateType.SPECIAL_STATE):
...
def subtokenize_identifier(identifier):
    ...
    # component.
    logging.vlog(
        _PEDANTIC, "Split %r into snake case: %r", identifier, snake_components
    )
    ...
    for snake_component in snake_components:
        if "_" in snake_component:
            ...
            logging.vlog(
                _PEDANTIC,
                "Split snake component %r into %r components.",
                unicodified_snake_component,
                camel_components,
            )
    ...
    assert six.ensure_text("".join(final_components)) == six.ensure_text(identifier), (
...
def split_long_token(token_string, max_output_token_length):
    ...
    for multi_token in token_lists:
        ...
        if multi_token.kind is TokenKind.IDENTIFIER:
            subtokenized = dataclasses.replace(
                multi_token, spellings=subtokenize_identifier(multi_token.spellings[0])
            )
...
def _subtokenize_strings_heuristically(token_lists):
...
def split_agnostic_tokens(
    ...
    agnostic_token_lists = _agnostic_tokens_to_lists_of_token_lists(agnostic_tokens)
...
def sanitize_subtoken_lists(subtoken_lists, sanitization_mapping, sentinel):
    ...
    if not subtoken_lists:
        raise ValueError(
            "Received empty input %r but expected it to be non "
            "empty" % subtoken_lists
        )
    if sentinel not in sanitization_mapping:
        raise ValueError(
            "Sentinel %r should be in the sanitization map %r "
    ...
    for multi_token in subtoken_lists:
        ...
        if not spellings:
            raise ValueError(
                "Received empty multi-token %r but expected no empty "
                "ones" % multi_token
            )
        sanitized_spellings = [sanitize(t, sanitization_mapping) for t in spellings]
...
def flatten_subtoken_lists(subtoken_lists):
...
def flatten_and_sanitize_subtoken_lists(subtoken_lists, sanitization_mapping, sentinel):
    ...
    Raises:
      ValueError: If one of the input sublists is empty, or the entire input
        is empty, or the sentinel is not one of the unsanitary characters.
    """
    sanitized = sanitize_subtoken_lists(subtoken_lists, sanitization_mapping, sentinel)
    flattened = flatten_subtoken_lists(sanitized)
    return flattened


def reconstitute_full_unsanitary_tokens(
    split_token_list, sanitization_mapping, sentinel
):
    ...
    for t in split_token_list:
        if not t:
            raise ValueError(
                "Must have non-empty subtokens, but found %r in %r."
                % (t, split_token_list)
            )
        if t[-1] == sentinel:
    ...
    if pending_split_tokens:
        raise ValueError(
            "After scanning all subtokens %r, there still is some "
            "unjoined content: %r" % (split_token_list, pending_split_tokens)
        )
    ...
    for whole_token in whole_token_list:
        for unsanitary_character in unsanitary_characters:
            if unsanitary_character in whole_token:
                raise ValueError(
                    "Reconstructed whole token %r seems to contain a "
...
def check_mappings(mappings):
    ...
    for key in mappings:
        if len(key) != 1:
            raise ValueError(
                "Expecting length-1 strings as keys in mappings, but "
    ...
    if len(values) != len(set(values)):
        raise ValueError(
            "There seem to be some duplicate values in %r, but they "
            "are expected to be unique." % mappings
        )

    if any([not value for value in values]):
        raise ValueError(
            "An empty value found in %r, but no empty values are " "allowed." % mappings
        )
    ...
    for value in values:
        for other_value in values:
            if value != other_value and value in other_value:
                raise ValueError(
                    "Value %r is a substring of %r, but no value may "
        ...
        for key in mappings:
            if key in value:
                raise ValueError(
                    "No key may appear in one of the mapping values, but "
...
def subtokenize_agnostic_tokens_in_place(
    ...
    """
    labelled_subtokenized = split_agnostic_tokens(
        agnostic_tokens, max_output_token_length
    )

    subtoken_lists = sanitize_subtoken_lists(
        labelled_subtokenized, sanitization_mapping, sentinel
    )
