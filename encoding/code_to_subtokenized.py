...
def next_whole_token(wordpiece_subtokens, initial_tokenizer, subword_tokenizer):
    ...
    if len(full_whole_tokens) < 2:
        ...
        raise ValueError(
            f"Whole tokens {full_whole_tokens} ended up "
    ...
    # full sequence
    raise ValueError(f"Could not find a whole token in {wordpiece_subtokens!r}")


def wordpiece_subtokens_to_code(
    wordpiece_subtokens, initial_tokenizer, subword_tokenizer
):
...
def code_to_cubert_sentences(
            ...
            if i == 0:
                ...
                if len(tokens) == 1:
                    ...
                    raise AssertionError(
                        "itertools.groupby seems to have returned an "
    ...
    sentences = [
        s + [unified_tokenizer.NEWLINE] for s in raw_sentences
    ]  # type: List[List[str]]
    logging.vlog(5, "Tokens are split into sentences: >>>%s<<<.", sentences)
    ...
    for sentence in sentences:
        encoded_tokens = [
            subword_tokenizer.encode_without_tokenizing(t) for t in sentence
        ]  # type: List[List[int]]
    ...
    logging.vlog(
        5, "Sentences are further subtokenized: >>>%s<<<.", subtokenized_sentences
    )
