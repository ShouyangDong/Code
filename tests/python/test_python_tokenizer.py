...
_EOS_NAME = unified_tokenizer.quote_special(unified_tokenizer.TokenKind.EOS.name)
_ERROR_NAME = unified_tokenizer.quote_special(unified_tokenizer.TokenKind.ERROR.name)
...
class PythonTokenizerTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ("nothing", "", ((0, 0, 0, 0, unified_tokenizer.TokenKind.EOS),)),
        (
            "same_line",
            """TokenA TokenB""",
            #  0     67
...
""",
    ...
    def test_python_tokenize_abstract_returns_positioning(
        ...
        actual_positions_and_kinds = tuple(
            (
                ...
                m.metadata.start.column,
                m.metadata.end.line,
                m.metadata.end.column,
                m.kind,
            )
            for m in agnostic
        )
        ...
        self.assertSequenceEqual(
            expected_positions_and_kinds, actual_positions_and_kinds
        )
    ...
    @parameterized.named_parameters(
        ...
        (
            ...
            [],
            "a\n\tb",
            ["a", "___NEWLINE___", "___INDENT___\t", "b", "___DEDENT___", _EOS_NAME],
        ),
        (
    ...
    def test_python_tokenize_returns_expected(self, types_to_skip, source, expected):
        tokenizer = python_tokenizer.PythonTokenizer()
   ...
   pass""",
    ...
    @parameterized.named_parameters(
        (
            "_comments",
            "def f():\n" "  # My comment\n" "  pass",
            "def f ():\n" "# My comment\n" "  pass ",  # Comments are not indented.
        ),
        (
