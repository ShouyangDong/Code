...
class SubtokenizeIdentifierTest(parameterized.TestCase):
    ...
    def test_subtokenize_identifier_returns_expected(self, identifier, components):
        actual_components = unified_tokenizer.subtokenize_identifier(identifier)
        self.assertListEqual(
            [six.ensure_text(c) for c in components],
...
class SanitizationTest(parameterized.TestCase):
    @parameterized.named_parameters(
        ...
        ("no_text", "", {"a": "ALPHA"}, ""),
        ("found", "abcabc", {"b": "BETA"}, "aBETAcaBETAc"),
        ("not_found", "abcabc", {"z": "ZETA"}, "abcabc"),
        (
            "two",
            "abcabc",
            {
    ...
    @parameterized.named_parameters(
        ("no_mappings", "abcabc", {}, "abcabc"),
        ("no_text", "", {"a": "ALPHA"}, ""),
        ("found", "aBETAcaBETAc", {"b": "BETA"}, "abcabc"),
        ("not_found", "abcabc", {"z": "ZETA"}, "abcabc"),
        (
            "two",
            "aBETAcaBETAc",
...
class SplitAgnosticTest(parameterized.TestCase):
    ...
    def test_split_agnostic_returns_expected(
        self, labelled_tokens, max_length, expected_labelled_subtokens
    ):
        tokens = [
            unified_tokenizer.AbstractToken(s, k, unified_tokenizer.TokenMetadata())
            for s, k in labelled_tokens
        ]
        labelled_subtokens = unified_tokenizer.split_agnostic_tokens(tokens, max_length)
...
class FlattenUnflattenTest(parameterized.TestCase):
    ...
    def test_flatten_returns_expected(
        self, subtoken_lists, mappings, expected_subtoken_list
    ):
        multi_tokens = []
    ...
    def test_reconstitute_returns_expected(self, subtokens, mappings, expected_tokens):
