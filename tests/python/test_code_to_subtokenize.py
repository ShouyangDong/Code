...
class CodeToSubtokenizedSentencesTest(parameterized.TestCase):
    ...
    def setUp(self):
        ...
        subword_vocabulary_path = self.create_tempfile(
            content=self._VOCABULARY_CONTENT
        ).full_path
    ...
    @parameterized.named_parameters(
        ...
        ("complete_cubert_before_incomplete", ("uv", "ww^_", "yz_", "xy"), "uvwwyz", 3),
        (
    ...
    def test_next_whole_token(
        self, subtokens, expected_whole_token, expected_end_index
    ):
        actual_whole_result = code_to_subtokenized_sentences.next_whole_token(
            subtokens, self.tokenizer, self.subword_text_encoder
        )
        self.assertEqual(
            (expected_whole_token, expected_end_index), actual_whole_result
        )
